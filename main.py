...
def main(
         ...
         llon, rlon,
         region_name,
         DATA_PATH='/tera05/lilu/ForDs/data/HH/'):    
    ...
    f = nc.Dataset(DATA_PATH+'DEM/MERITDEM/MERITDEM_aspect.nc', 'r')
    aspect_fine = f['asp'][lat_fine_index][:,lon_fine_index]
    f = nc.Dataset(DATA_PATH+'DEM/MERITDEM/MERITDEM_slope.nc', 'r')
    slope_fine = f['slp'][lat_fine_index][:,lon_fine_index]
    f = nc.Dataset(DATA_PATH+'DEM/MERITDEM/MERITDEM_curvature.nc', 'r')
    curvature_fine = f['curvature'][lat_fine_index][:,lon_fine_index]
    f = nc.Dataset(DATA_PATH+'DEM/MERITDEM/MERITDEM_SkyViewFactor.nc', 'r')
    svf_fine = f['svf'][lat_fine_index][:,lon_fine_index]
    #f = nc.Dataset(DATA_PATH+'DEM/MERITDEM/shadowmask_LUT_Heihe.nc','r')
    #shadow_mask_LUT = f['shadowmask_1'][lat_fine_index][:,lon_fine_index]
    f = nc.Dataset(DATA_PATH+'DEM/MERITDEM/shadowmask_LUT_Heihe_front.nc','r')
    shadow_mask_LUT_front = f['shadowmask_front_1'][lat_fine_index][:,lon_fine_index]
    f = nc.Dataset(DATA_PATH+'DEM/MERITDEM/shadowmask_LUT_Heihe_back.nc','r')
    shadow_mask_LUT_back = f['shadowmask_back_1'][lat_fine_index][:,lon_fine_index]


    shadow_mask_LUT = np.load(DATA_PATH+'DEM/MERITDEM/shadow_mask_LUT.npy')[lat_fine_index][:,lon_fine_index]
    #f = nc.Dataset(DATA_PATH+'DEM/MERITDEM/MERITDEM_ShadowMaskFactor.nc', 'r')
    #shadow_mask_LUT = f['sm'][lat_fine_index][:,lon_fine_index]
    ...
    f = nc.Dataset(DATA_PATH+'forcing/ERA5_HH_{year:04}_{month:02}_t2m_interp.nc'.format(year=year, month=month), 'r')
    t2m_fine_interp = f['t2m'][day_of_month*24:day_of_month*24+24][:,lat_fine_index][:,:,lon_fine_index]
    f = nc.Dataset(DATA_PATH+'forcing/ERA5_HH_{year:04}_{month:02}_t2m.nc'.format(year=year, month=month), 'r')
    ...
    f = nc.Dataset(DATA_PATH+'forcing/ERA5_HH_{year:04}_{month:02}_d2m_interp.nc'.format(year=year, month=month), 'r')
    d2m_fine_interp = f['d2m'][day_of_month*24:day_of_month*24+24][:,lat_fine_index][:,:,lon_fine_index]
    f = nc.Dataset(DATA_PATH+'forcing/ERA5_HH_{year:04}_{month:02}_d2m.nc'.format(year=year, month=month), 'r')
    ...
    print('processing sp')
    f = nc.Dataset(DATA_PATH+'forcing/ERA5_HH_{year:04}_{month:02}_sp_interp.nc'.format(year=year, month=month), 'r')
    ...
    f = nc.Dataset(DATA_PATH+'forcing/ERA5_HH_{year:04}_{month:02}_msdwlwrf_interp.nc'.format(year=year, month=month), 'r')    
    strd_fine_interp = f['msdwlwrf'][day_of_month*24:day_of_month*24+24][:,lat_fine_index][:,:,lon_fine_index]    
    f = nc.Dataset(DATA_PATH+'forcing/ERA5_HH_{year:04}_{month:02}_msdwlwrf.nc'.format(year=year, month=month), 'r')
    ...
    f = nc.Dataset(DATA_PATH+'forcing/ERA5_HH_{year:04}_{month:02}_u10_interp.nc'.format(year=year, month=month), 'r')
    u10_fine_interp = f['u10'][day_of_month*24:day_of_month*24+24][:,lat_fine_index][:,:,lon_fine_index] 
    f = nc.Dataset(DATA_PATH+'forcing/ERA5_HH_{year:04}_{month:02}_v10_interp.nc'.format(year=year, month=month), 'r')
    ...
    f = nc.Dataset(DATA_PATH+'forcing/ERA5_HH_{year:04}_{month:02}_msdwswrf.nc'.format(year=year, month=month), 'r')
    ssrd_coarse = f['msdwswrf'][day_of_month*24:day_of_month*24+24,:,:][:,lat_coarse_index][:,:,lon_coarse_index]
    ssrd_coarse[ssrd_coarse<0] = 0
    f = nc.Dataset(DATA_PATH+'forcing/ERA5_HH_{year:04}_{month:02}_sp.nc'.format(year=year, month=month), 'r')
    ...
    ssrd_fine = downscale_in_shortwave_radiation(
                                            ...
                                            lon_fine,
                                            shadow_mask_LUT,
                                            shadow_mask_LUT_front,
                                            shadow_mask_LUT_back)
    save2nc('msdwswrf', year, month, day_of_month, ssrd_fine, lat_fine, lon_fine)
    """
    """
    # downscale precipitation by colm
    f = nc.Dataset(DATA_PATH+'forcing/ERA5_HH_{year:04}_{month:02}_mtpr.nc'.format(year=year, month=month), 'r')
