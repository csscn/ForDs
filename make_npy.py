...
def pred_automl(args, day_of_month, region_name):
    ...
    f_t = xr.open_dataset("/tera05/lilu/ForDs/ERA5_DS/HRB/t2m/ERA5_{region_name}_fine_{year:04}_{month:02}_t2m.nc".format(
                  year=args.year, month=args.month, region_name=region_name))
    f_q = xr.open_dataset("/tera05/lilu/ForDs/ERA5_DS/HRB/q/ERA5_{region_name}_fine_{year:04}_{month:02}_q.nc".format(
                  year=args.year, month=args.month, region_name=region_name))
    f_d2m = xr.open_dataset("/tera05/lilu/ForDs/ERA5_DS/HRB/d2m/ERA5_{region_name}_fine_{year:04}_{month:02}_d2m.nc".format(
                  year=args.year, month=args.month, region_name=region_name))
    f_sp = xr.open_dataset("/tera05/lilu/ForDs/ERA5_DS/HRB/sp/ERA5_{region_name}_fine_{year:04}_{month:02}_sp.nc".format(
                  year=args.year, month=args.month, region_name=region_name))
    f_sw = xr.open_dataset("/tera05/lilu/ForDs/ERA5_DS/HRB/msdwswrf/ERA5_{region_name}_fine_{year:04}_{month:02}_msdwswrf.nc".format(
                  year=args.year, month=args.month, region_name=region_name))
    f_lw = xr.open_dataset("/tera05/lilu/ForDs/ERA5_DS/HRB/msdwlwrf/ERA5_{region_name}_fine_{year:04}_{month:02}_msdwlwrf.nc".format(
                  year=args.year, month=args.month, region_name=region_name))
    #f_ws =xr.open_dataset("/tera05/lilu/ForDs/ERA5_DS/HRB/ws/ERA5_{region_name}_fine_{year:04}_{month:02}_ws.nc".format(
    #             year=args.year, month=args.month, region_name=region_name))
    f_ws_modify = xr.open_dataset("/tera05/lilu/ForDs/ERA5_DS/HRB/ws_modify/ERA5_{region_name}_fine_{year:04}_{month:02}_ws.nc".format(
                  year=args.year, month=args.month, region_name=region_name),decode_times=False)
    
    ws_modify = np.array(f_ws_modify["ws"].data[(day_of_month-1)*24:(day_of_month-1)*24+24])
    #ws = np.array(f_ws["ws"].data[(day_of_month-1)*24:(day_of_month-1)*24+24])
    print("ws.shape:",ws_modify.shape)
    t2m = np.array(f_t["t2m"].data[(day_of_month-1)*24:(day_of_month-1)*24+24])
    q = np.array(f_q["q"].data[(day_of_month-1)*24:(day_of_month-1)*24+24])
    d2m = np.array(f_d2m["d2m"].data[(day_of_month-1)*24:(day_of_month-1)*24+24])
    sp = np.array(f_sp["sp"].data[(day_of_month-1)*24:(day_of_month-1)*24+24])
    sw = np.array(f_sw["msdwswrf"].data[(day_of_month-1)*24:(day_of_month-1)*24+24])
    lw = np.array(f_lw["msdwlwrf"].data[(day_of_month-1)*24:(day_of_month-1)*24+24])
    downscale_precipitation(t2m, d2m, sp, q, lw, sw, ws_modify, lat_fine, lon_fine, elev_fine, args.year, args.month, day_of_month)
    """
    x = np.load('x_test/{region_name}/x_test_{year:04}_{month}_{day}.npy'.format(
...
def par_pred_automl(args):
    ...
    for i in range(args.begin_day, args.end_day):
        #for j in range(150):
        job = Process(target=pred_automl,  args=(args, i, args.region_name))
        job.start()
    job.join()
