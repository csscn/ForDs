...
# diffuse radiation scheme 1 based on assumption of isotropic scattering : S^=S*Vd
def scheme1(sky_view_factor_fine,diffuse_radiation_fine_interp):
    diffuse_radiation_fine = sky_view_factor_fine*diffuse_radiation_fine_interp
    return diffuse_radiation_fine

# diffuse radiation scheme 2 consider antisotropic scattering. From Huang, A. et al., .(2022). 
...
def scheme2(sky_view_factor_fine,diffuse_radiation_fine_interp,cos_illumination,beam_radiation_fine,toa_in_short_radiation_fine_interp,\
    slope_fine,beam_radiation_fine_interp):
    ...
    for i in range(24):
        ...
        ratio3[idx4] = 0
        ratio3[ratio3<0] = 0
        diffuse_radiation_fine[i][idx2] = diffuse_radiation_fine_interp[i][idx2]*(0.5*sky_view_factor_fine[idx2]*(1+np.cos(slope_fine[idx2]))*(1-ratio3))
...
def downscale_in_shortwave_radiation(in_short_radiation_coarse,
                                     ...
                                     lon_fine,
                                     lut_shadow_mask,
                                     lut_shadow_mask_front,
                                     lut_shadow_mask_back):
    # ------------------------------------------------------------
    ...
    for i in range(24):
        for n in range(idx.shape[1]):
            for m in range(idx.shape[2]):
                """ 
                sita1=np.arcsin(lut_shadow_mask_front[n,m,idx[i,n,m]])
                sita2=np.arcsin(lut_shadow_mask_back[n,m,idx[i,n,m]])
                if np.pi/2-zenith_angle_fine[i,n,m] < sita2:
                    shadow_mask[i,n,m] = 0
                elif np.pi/2-zenith_angle_fine[i,n,m]> sita1:
                    shadow_mask[i,n,m] =1
                else:
                    shadow_mask[i,n,m] = ((np.pi/2-zenith_angle_fine[i,n,m])-sita2)/(sita1-sita2)
    ...
    cos_illumination_origin = cos_illumination 
    ...
    #diffuse_radiation_fine = sky_view_factor_fine*diffuse_radiation_fine_interp
    #diffuse_radiation_fine = scheme2(sky_view_factor_fine,diffuse_radiation_fine_interp,cos_illumination_origin,beam_radiation_fine,toa_in_short_radiation_fine_interp,slope_fine,beam_radiation_fine_interp)
    #diffuse_radiation_fine = scheme1(sky_view_factor_fine,diffuse_radiation_fine_interp)
    diffuse_radiation_fine = diffuse_radiation_fine_interp
    # ------------------------------------------------------------
    ...
    """ 
    np.save('in_short_radiation_coarse_{month}_{day}1'.format(month=month, day=day_of_month), np.array(in_short_radiation_coarse))
    np.save('beam_radiation_coarse_{month}_{day}1'.format(month=month, day=day_of_month), np.array(beam_radiation_coarse))
    np.save('diffuse_radiation_coarse_{month}_{day}1'.format(month=month, day=day_of_month), np.array(diffuse_radiation_coarse))
    np.save('beam_radiation_fine_interp_{month}_{day}1'.format(month=month, day=day_of_month), np.array(beam_radiation_fine_interp))
    np.save('diffuse_radiation_fine_interp_{month}_{day}1'.format(month=month, day=day_of_month), np.array(diffuse_radiation_fine_interp))
    np.save('cos_illumination_{month}_{day}1'.format(month=month, day=day_of_month), np.array(cos_illumination))
    np.save('shadow_mask_{month}_{day}1'.format(month=month, day=day_of_month), np.array(shadow_mask))
    np.save('optical_length_factor_fine_{month}_{day}1'.format(month=month, day=day_of_month), np.array(optical_length_factor_fine))
    np.save('diffuse_radiation_fine_{month}_{day}1'.format(month=month, day=day_of_month), np.array(diffuse_radiation_fine))
    np.save('reflected_radiation_fine_{month}_{day}1'.format(month=month, day=day_of_month), np.array(reflected_radiation_fine))
    """
    return np.array(cos_illumination)*np.array(shadow_mask)*np.array(beam_radiation_fine_interp)+np.array(diffuse_radiation_fine)#*np.array(optical_length_factor_fine)+\
...
def downscale_precipitation_colm(precipitation_coarse,
    ...
    print(precipitation_coarse.shape, scale)
...
def downscale_precipitation(
    ...
    print("julianday.shape: ",julian_day.shape)
    print("elev_fine.shape: ",elev_fine.shape)
    # downscaling
